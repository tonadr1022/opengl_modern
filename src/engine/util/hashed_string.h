...
// copied from:
// https://github.com/JuanDiegoMontoya/Gengine/blob/37739ecfcb608f6b282b36ed8a962fac968b1487/src/utility/HashedString.h
...
class basic_hashed_string {
  ...
  [[nodiscard]] static constexpr uint32_t helper(const Char* curr) noexcept {
    ...
    while (*curr != 0) {
      value =
          (value ^ static_cast<traits_type::type>(std::tolower(*(curr++)))) * traits_type::prime;
    }
 ...
 public:
   ...
   */
  [[nodiscard]] constexpr const value_type* data() const noexcept { return str; }
...
struct hash<HashedString> {
  std::size_t operator()(const HashedString& hs) const noexcept { return hs.value(); }
};
